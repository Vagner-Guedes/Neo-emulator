...
Add-ChecklistItem $items 'backend-qemu-whpx' 'QEMU x86_64 com WHPX configurado' 'config/runtime.json + diagnostics.json + launcher-smoke.json' ($configBackend -and $configWhpx -and $configNoAndroidStudio -and (Test-ReportFresh $diagnostics) -and (Test-ReportFresh $launcherSmoke) -and (Has-PropertyValue $diagnostics 'tools.backend' 'QEMU Android-x86') -and (Has-PropertyValue $diagnostics 'tools.whpx.available' $true) -and (Has-PropertyValue $launcherSmoke 'manualEvidence.qemuNoConsoleObserved' $true)) 'Configuração e WHPX devem coincidir; o diagnóstico e a observação live precisam confirmar QEMU sem console.' ($null -ne $diagnostics -or $null -ne $launcherSmoke)
Add-ChecklistItem $items 'guest-identity' 'Android-x86 7.1.2/API 25 confirmado' 'diagnostics.json ou nativebridge.json' ($configIdentity -and ((Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'android.identityMatches' $true) -or (Test-ReportFresh $native) -and (Has-PropertyValue $native 'guestIdentityMatches' $true))) 'Release, API e sys.boot_completed precisam ser confirmados no guest.' ($null -ne $diagnostics -or $null -ne $native)
Add-ChecklistItem $items 'persistent-disk' 'qcow2 persistente configurado e reutilizado' 'config/runtime.json + qemu-persistence.json + launcher-smoke.json' ($configDisk -and (Test-ReportFresh $persistence) -and (Test-ReportFresh $launcherSmoke) -and (Has-PropertyValue $persistence 'status' 'validated') -and (Has-PropertyValue $launcherSmoke 'manualEvidence.windowsRestartObserved' $true)) 'O marcador deve sobreviver a processos QEMU; a persistência após reinício do Windows requer observação explícita.' ($null -ne $persistence -or $null -ne $launcherSmoke)
Add-ChecklistItem $items 'adb-tcp' 'ADB TCP host 127.0.0.1:5556 para guest 5555' 'config/runtime.json + diagnostics.json' ($configAdb -and (Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'android.adb.serial' '127.0.0.1:5556')) 'O serial e o encaminhamento precisam ser observados no runtime.' ($null -ne $diagnostics)
Add-ChecklistItem $items 'qmp-shutdown' 'Desligamento QEMU via QMP sem adb emu kill' 'qemu-benchmark.json ou qemu-persistence.json' (((Test-ReportFresh $benchmark) -and (Has-PropertyValue $benchmark 'acceleration' 'whpx') -and (Has-PropertyValue $benchmark 'runs.0.stopped' $true)) -or ((Test-ReportFresh $persistence) -and (Has-PropertyValue $persistence 'acceleration' 'whpx') -and (Has-PropertyValue $persistence 'firstShutdownSucceeded' $true))) 'O relatório live deve registrar encerramento controlado.' ($null -ne $benchmark -or $null -ne $persistence)
Add-ChecklistItem $items 'native-bridge-property' 'Native Bridge ARM declarado e operacional' 'nativebridge.json ou diagnostics.json' ($configNative -and (((Test-ReportFresh $native) -and (Has-PropertyValue $native 'nativeBridgeReady' $true)) -or ((Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'abiCompatibility.nativeBridgeReady' $true)))) 'Property isolada não basta; o relatório precisa incluir a execução do APK.' ($null -ne $native -or $null -ne $diagnostics)
Add-ChecklistItem $items 'official-apk-identity' 'APK oficial, pacote, versão e activity corretos' 'config/runtime.json + diagnostics.json' ($configApk -and (Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'tools.requiredFiles.neoNewsApk.exists' $true)) 'A presença do arquivo é pré-condição; a assinatura é validada separadamente.' ($null -ne $diagnostics)
Add-ChecklistItem $items 'official-apk-signature' 'Certificado SHA-256 autorizado confirmado' 'diagnostics.json' ((Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'abiCompatibility.signature.valid' $true)) 'O certificado observado deve coincidir com a fingerprint configurada.' ($null -ne $diagnostics)
Add-ChecklistItem $items 'official-apk-abis' 'APK contém arm64-v8a e armeabi-v7a' 'diagnostics.json' ((Test-ReportFresh $diagnostics) -and (Has-PropertyValue $diagnostics 'abiCompatibility.apkAbis' { param($v) @($v) -contains 'arm64-v8a' -and @($v) -contains 'armeabi-v7a' })) 'As duas ABIs ARM do APK oficial devem ser observadas.' ($null -ne $diagnostics)
Add-ChecklistItem $items 'apk-install' 'adb install -r retornou Success e preservou dados' 'nativebridge.json + launcher-smoke.json' ((Test-ReportFresh $native) -and (Test-ReportFresh $launcherSmoke) -and (Has-PropertyValue $native 'installSucceeded' $true) -and (Has-PropertyValue $launcherSmoke 'manualEvidence.updatePreservationObserved' $true)) 'A instalação deve retornar Success e a atualização deve preservar dados, sem uninstall ou clear data.' ($null -ne $native -or $null -ne $launcherSmoke)
Add-ChecklistItem $items 'abi-selection' 'primaryCpuAbi ARM32 selecionada pela instalação' 'nativebridge.json' ((Test-ReportFresh $native) -and (Has-PropertyValue $native 'selectedApkAbi' { param($v) [string]$v -eq [string]$config.android.preferredApkAbi }) -and (Has-PropertyValue $native 'primaryCpuAbi' { param($v) [string]$v -eq [string]$config.android.preferredApkAbi })) 'A ABI selecionada precisa ser exatamente armeabi-v7a e coincidir com o package dump.' ($null -ne $native)
Add-ChecklistItem $items 'activity-launch' 'TerminalActivity abriu no guest' 'nativebridge.json' ((Test-ReportFresh $native) -and (Has-PropertyValue $native 'launchSucceeded' $true) -and (Has-PropertyValue $native 'activityRunning' $true)) 'O launch e a atividade em primeiro plano precisam ser confirmados.' ($null -ne $native)
Add-ChecklistItem $items 'initial-stability' 'NeoNews permaneceu estável após o primeiro launch' 'nativebridge.json' ((Test-ReportFresh $native) -and (Has-PropertyValue $native 'runtimeStable' $true)) 'Logcat filtrado não pode conter falhas de linker, WebView ou crash.' ($null -ne $native)
Add-ChecklistItem $items 'restart-stability' 'NeoNews sobreviveu a reinício do guest' 'nativebridge.json' ((Test-ReportFresh $native) -and (Has-PropertyValue $native 'restartCount' { param($v) [int]$v -ge 1 }) -and (Has-PropertyValue $native 'restartResults' { param($v) @($v).Count -ge 1 -and @($v | Where-Object { $_.stable -ne $true }).Count -eq 0 })) 'Cada reinício deve voltar a boot, launch e estabilidade.' ($null -ne $native)
...
Add-ChecklistItem $items 'webview-package' 'Pacote WebView esperado está instalado' 'webview-provider.json' ((Test-ReportFresh $webViewProvider) -and (Has-PropertyValue $webViewProvider 'provider.packagePresent' $true)) 'Provider ausente não pode ser tratado como pronto.' ($null -ne $webViewProvider)
Add-ChecklistItem $items 'webview-active' 'Provider WebView esperado está ativo' 'webview-provider.json' ((Test-ReportFresh $webViewProvider) -and (Has-PropertyValue $webViewProvider 'provider.providerActive' $true)) 'A linha Current WebView package deve apontar ao provider esperado.' ($null -ne $webViewProvider)
Add-ChecklistItem $items 'webview-version' 'WebView ativo é 119.0.6045.193' 'webview-provider.json ou webview-content.json' ($configWebView -and (((Test-ReportFresh $webViewProvider) -and (Has-PropertyValue $webViewProvider 'provider.versionMatches' $true)) -or ((Test-ReportFresh $webViewContent) -and (Has-PropertyValue $webViewContent 'provider.versionMatches' $true)))) 'A versão instalada deve coincidir exatamente com a configuração.' ($null -ne $webViewProvider -or $null -ne $webViewContent)
Add-ChecklistItem $items 'webview-native-abi' 'WebView possui ABI nativa x86/x86_64' 'webview-provider.json ou webview-content.json' (((Test-ReportFresh $webViewProvider) -and (Has-PropertyValue $webViewProvider 'provider.nativeAbiMatches' $true)) -or ((Test-ReportFresh $webViewContent) -and (Has-PropertyValue $webViewContent 'provider.nativeAbiMatches' $true))) 'Não aceitar provider ARM-only no guest x86.' ($null -ne $webViewProvider -or $null -ne $webViewContent)
Add-ChecklistItem $items 'webview-html-css-js' 'WebView executa HTML, CSS e JavaScript reais' 'webview-content.json' ((Test-ReportFresh $webViewContent) -and (Has-PropertyValue $webViewContent 'content.validated' $true) -and (Has-PropertyValue $webViewContent 'content.result' { param($v) [string]$v -match '(?i)localHtml=True.*localCss=True.*localJs=True' })) 'O probe deve executar conteúdo local e retornar as três fases.' ($null -ne $webViewContent)
Add-ChecklistItem $items 'webview-https' 'WebView carrega conteúdo HTTPS real' 'webview-content.json' ((Test-ReportFresh $webViewContent) -and (Has-PropertyValue $webViewContent 'content.validated' $true) -and (Has-PropertyValue $webViewContent 'content.result' { param($v) [string]$v -match '(?i)remoteHttps=True.*remoteContent=True' })) 'HTTPS e corpo não vazio devem ser observados.' ($null -ne $webViewContent)
Add-ChecklistItem $items 'rhvoice-provider' 'RHVoice é engine padrão e locale pt-BR disponível' 'tts-synthesis.json ou tts-provider.json' ($configTts -and (Test-ReportFresh $ttsSynthesis) -and (Test-ReportFresh $ttsProvider) -and (Has-PropertyValue $ttsSynthesis 'defaultEngineMatches' $true) -and (Has-PropertyValue $ttsSynthesis 'probeLocale' $true) -and (Has-PropertyValue $ttsProvider 'status' { param($v) [string]$v -like 'provider-and-locale-validated*' })) 'Provider, engine selecionada e dados de locale devem coincidir.' ($null -ne $ttsSynthesis -or $null -ne $ttsProvider)
Add-ChecklistItem $items 'rhvoice-audio' 'RHVoice produziu áudio não vazio' 'tts-synthesis.json' ((Test-ReportFresh $ttsSynthesis) -and (Has-PropertyValue $ttsSynthesis 'audio.nonEmpty' $true) -and (Has-PropertyValue $ttsSynthesis 'status' 'validated')) 'Presença do pacote não é prova de síntese.' ($null -ne $ttsSynthesis)
Add-ChecklistItem $items 'network-online' 'DNS, HTTP e HTTPS funcionam no guest' 'guest-network-media.json' ((Test-ReportFresh $networkMedia) -and (Has-PropertyValue $networkMedia 'online.validated' $true) -and (Has-PropertyValue $networkMedia 'online.raw' { param($v) [string]$v -match '(?i)dns=true.*http=true.*https=true' })) 'Use URLs aprovadas pelo ambiente.' ($null -ne $networkMedia)
