...
Add-ChecklistItem $items 'qmp-shutdown' 'Desligamento QEMU via QMP sem adb emu kill' 'qemu-benchmark.json ou qemu-persistence.json' (((Test-ReportFresh $benchmark) -and (Test-QemuReportIdentity $benchmark) -and (Has-PropertyValue $benchmark 'runs' { param($v) @($v).Count -gt 0 -and @($v | Where-Object { $_.stopped -ne $true -or $_.qmpShutdownSucceeded -ne $true -or $_.forcedKill -eq $true }).Count -eq 0 })) -or ((Test-ReportFresh $persistence) -and (Test-QemuReportIdentity $persistence) -and (Has-PropertyValue $persistence 'firstShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'firstQmpShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'firstForcedKill' $false) -and (Has-PropertyValue $persistence 'secondShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'secondQmpShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'secondForcedKill' $false))) 'O relatório live deve confirmar a negociação QMP, o envio de quit, a saída de cada processo sem kill forçado e o backend/serial configurados.' ($null -ne $benchmark -or $null -ne $persistence)
