...
Add-ChecklistItem $items 'backend-qemu-whpx' 'QEMU x86_64 com WHPX configurado' 'config/runtime.json + diagnostics.json' ($configBackend -and $configWhpx -and (Has-PropertyValue $diagnostics 'tools.backend' 'QEMU Android-x86') -and (Has-PropertyValue $diagnostics 'tools.whpx.available' $true)) 'Configuração e WHPX devem coincidir; diagnóstico live precisa confirmar a biblioteca.' ($null -ne $diagnostics)
...
Add-ChecklistItem $items 'persistent-disk' 'qcow2 persistente configurado e reutilizado' 'config/runtime.json + qemu-persistence.json' ($configDisk -and (Has-PropertyValue $persistence 'status' 'validated')) 'O marcador deve sobreviver a dois processos QEMU com o mesmo disco.' ($null -ne $persistence)
...
Add-ChecklistItem $items 'apk-install' 'adb install -r retornou Success' 'nativebridge.json' (Has-PropertyValue $native 'installSucceeded' $true) 'Sem uninstall, clear data ou substituição do APK.' ($null -ne $native)
...
Add-ChecklistItem $items 'kiosk-display' 'Kiosk aplicou display, density e rotação configurados' 'config/runtime.json + diagnostics.json' $configKiosk 'A configuração é necessária; diagnóstico live deve conter os valores do guest.' ($null -ne $config)
...
Add-ChecklistItem $items 'watchdog' 'Watchdog ativo e com política de recuperação' 'diagnostics.json' ((Has-PropertyValue $diagnostics 'watchdog.active' $true) -and [bool]$config.supervisor.restartOnActivityLoss) 'A atividade do watchdog deve ser observada no diagnóstico.' ($null -ne $diagnostics)
...
Add-ChecklistItem $items 'launcher-cli-single-instance' 'Launcher, CLI, tray e instância única funcionam' 'launcher-smoke.json' (Has-PropertyValue $launcherSmoke 'status' 'validated') 'O smoke deve confirmar janela responsiva, segunda invocação e --exit sem residual.' ($null -ne $launcherSmoke)
...
if ($kioskItem) {
    $kioskLive = (Has-PropertyValue $diagnostics 'android.displaySize' { param($v) [string]$v -match [regex]::Escape([string]$config.android.kiosk.displaySize) }) -and
        ...
        (Has-PropertyValue $diagnostics 'android.kiosk.rotation' { param($v) [string]$v -match [regex]::Escape([string]$config.android.kiosk.rotation) })
    $kioskItem.status = if ($null -eq $launcherSmoke -or $null -eq $diagnostics) { 'pending' } elseif ($configKiosk -and $kioskLive -and (Has-PropertyValue $launcherSmoke 'manualEvidence.trayObserved' $true)) { 'pass' } else { 'fail' }
}
