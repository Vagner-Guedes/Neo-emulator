...
$noVisibleConsole = Read-Report 'no-visible-console.json'
...
$reportFreshness = [ordered]@{
    ...
    noVisibleConsole = Get-ReportFreshness $noVisibleConsole
...
Add-ChecklistItem $items 'qmp-shutdown' 'Desligamento QEMU via QMP sem adb emu kill' 'qemu-benchmark.json ou qemu-persistence.json' (((Test-ReportFresh $benchmark) -and (Test-QemuReportIdentity $benchmark) -and (Has-PropertyValue $benchmark 'runs' { param($v) @($v).Count -gt 0 -and @($v | Where-Object { $_.stopped -ne $true -or $_.qmpCapabilitiesSucceeded -ne $true -or $_.qmpQuitSent -ne $true -or $_.qmpQuitResponseSucceeded -ne $true -or $_.qmpShutdownSucceeded -ne $true -or $_.forcedKill -eq $true }).Count -eq 0 })) -or ((Test-ReportFresh $persistence) -and (Test-QemuReportIdentity $persistence) -and (Has-PropertyValue $persistence 'firstShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'firstQmpCapabilitiesSucceeded' $true) -and (Has-PropertyValue $persistence 'firstQmpQuitSent' $true) -and (Has-PropertyValue $persistence 'firstQmpShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'firstQmpQuitResponseSucceeded' $true) -and (Has-PropertyValue $persistence 'firstForcedKill' $false) -and (Has-PropertyValue $persistence 'secondShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'secondQmpCapabilitiesSucceeded' $true) -and (Has-PropertyValue $persistence 'secondQmpQuitSent' $true) -and (Has-PropertyValue $persistence 'secondQmpShutdownSucceeded' $true) -and (Has-PropertyValue $persistence 'secondQmpQuitResponseSucceeded' $true) -and (Has-PropertyValue $persistence 'secondForcedKill' $false))) 'O relatório live deve confirmar a negociação QMP, o envio de quit, a resposta positiva do quit, a saída de cada processo sem kill forçado e o backend/serial configurados.' ($null -ne $benchmark -or $null -ne $persistence)
...
Add-ChecklistItem $items 'official-apk-identity' 'APK oficial, pacote, versão e activity corretos' 'config/runtime.json + diagnostics.json' ($configApk -and (Test-ReportFresh $diagnostics) -and (Test-DiagnosticsIdentity $diagnostics) -and (Has-PropertyValue $diagnostics 'tools.requiredFiles.neoNewsApk.exists' $true) -and (Has-PropertyValue $diagnostics 'apk.packageName' ([string]$config.neonews.packageName)) -and (Has-PropertyValue $diagnostics 'apk.versionName' ([string]$config.neonews.versionName)) -and (Has-PropertyValue $diagnostics 'apk.versionCode' ([int]$config.neonews.versionCode))) 'A identidade do manifesto deve coincidir com package, versão e versionCode configurados; a activity é confirmada no gate de lançamento e a assinatura separadamente.' ($null -ne $diagnostics)
...
Add-ChecklistItem $items 'no-visible-console' 'Nenhum console técnico é visível durante as operações do produto' 'no-visible-console.json' ((Test-ReportFresh $noVisibleConsole) -and (Has-PropertyValue $noVisibleConsole 'gate' 'NO_VISIBLE_CONSOLE_PASS') -and (Has-PropertyValue $noVisibleConsole 'status' 'validated')) 'O gate exige contratos estáticos, observação real do launcher e evidência visual de instalação, boot, startup, runtime, QEMU, ADB, Guardian/recovery, restart, update, F11, F12 e uninstall/upgrade quando aplicável.' ($null -ne $noVisibleConsole -and [string]$noVisibleConsole.status -ne 'pending-evidence')
