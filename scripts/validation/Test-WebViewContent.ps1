...
$providerActive = $webViewDump -match [regex]::Escape([string]$config.webView.provider)
...
$providerValidated = $providerActive -and $versionMatches
...
$result = [ordered]@{
    ...
    serial = $Serial
    provider = [ordered]@{ packageName = $config.webView.provider; expectedVersion = $config.webView.homologatedVersion; installedVersion = $installedVersion; providerActive = $providerActive; versionMatches = $versionMatches }
