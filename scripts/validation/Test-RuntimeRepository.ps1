...
$adbSourcePath = Join-Path $RepositoryRoot 'launcher\NeoNews.Runtime.Launcher\Services\AdbService.cs'
$adbSource = if (Test-Path -LiteralPath $adbSourcePath) { Get-Content -LiteralPath $adbSourcePath -Raw -Encoding utf8 } else { '' }
...
$contractChecks = [ordered]@{
    ...
    operationalHotkeysUseReservedCombinations = [string]$configObject.runtime.stopRuntimeHotkey -eq 'Ctrl+Alt+Shift+F11' -and [string]$configObject.runtime.toggleFullscreenHotkey -eq 'Ctrl+Alt+Shift+F12' -and [string]$configObject.runtime.stopRuntimeHotkey -notmatch '^(F11|F12)$' -and [string]$configObject.runtime.toggleFullscreenHotkey -notmatch '^(F11|F12)$'
    operationalHotkeysHaveReservedKeyFallback = $launcherSourceText -match 'WhKeyboardLl' -and $launcherSourceText -match 'SetWindowsHookEx' -and $launcherSourceText -match 'RegisterRawInputDevices' -and $launcherSourceText -match 'RidevInputSink' -and $launcherSourceText -match 'IsRawKeyboardKey'
    ...
    provisioningPreservesStrongDiskHash = $launcherSourceText -match 'BaselineSha256' -and $launcherSourceText -match 'ImageHash'
    persistentDiskSeparatesBaselineAndActiveMetadata = $launcherSourceText -match 'BaselineSha256' -and $launcherSourceText -match 'ActiveDiskMetadata' -and $launcherSourceText -match 'EXPECTED_PERSISTENT_MUTATION' -and $launcherSourceText -match 'UNEXPECTED_IMAGE_MUTATION' -and $launcherSourceText -match 'qemu-img'
    persistentDiskStructuralCheckIsExplicit = $launcherSourceText -match 'check.*--output=json' -and $launcherSourceText -match 'BackingFile' -and $launcherSourceText -match 'CheckErrors' -and $baseProvisioningSource -match 'Invoke-QemuImgJson' -and $qemuBenchmarkCommonSource -match 'Get-QemuBenchmarkImageJson'
    ...
    guestConfigurationPinsUiGuards = [bool]$configObject.android.guestConfiguration.ui.disableTaskbar -and [string]$configObject.android.guestConfiguration.ui.taskbarPackageName -eq 'com.farmerbb.taskbar.androidx86' -and [string]$configObject.android.guestConfiguration.ui.immersiveModeConfirmation -eq 'confirmed' -and [bool]$configObject.android.guestConfiguration.disableNeoNewsBootReceiver -and [string]$configObject.android.guestConfiguration.neoNewsBootReceiver -eq 'com.in9midia.neonews.player/.NeonewsAutoStart' -and $launcherSourceText -match 'pm", "disable-user' -and $launcherSourceText -match 'pm", "disable"' -and $launcherSourceText -match 'immersive_mode_confirmations'
    earlyBootAppliesUiGuards = $launcherSourceText -match 'EARLY_UI_GUARD' -and $launcherSourceText -match 'TryApplyEarlyUiGuardsAsync' -and $launcherSourceText.IndexOf('TryApplyEarlyUiGuardsAsync') -lt $launcherSourceText.IndexOf('TryEnsureAndroidSetupCompleteAsync')
    qemuStartsFullscreenByDefault = [bool]$configObject.android.qemu.startFullscreen -and $qemuSource -match 'full-screen=on' -and $qemuBenchmarkCommonSource -match 'full-screen=on'
    clockWaitsForTimezoneBeforeHostWallClock = $adbSource -match 'Task.Delay\(TimeSpan.FromSeconds\(2\)' -and $adbSource -match 'hostBeforeSet\.LocalDateTime\.ToString'
    guestConfigurationCompletesAndroidSetup = $launcherSourceText -match 'device_provisioned' -and $launcherSourceText -match 'user_setup_complete' -and $launcherSourceText -match 'TryEnsureAndroidSetupCompleteAsync' -and $componentProvisioningSource -match 'Ensure-AndroidSetupComplete'
    setupWizardAndCrashDialogGates = $launcherSourceText -match 'SETUP_WIZARD_SUPPRESSED_PASS' -and $launcherSourceText -match 'NO_ANDROID_CRASH_DIALOG_PASS' -and $checklistSource -match 'android-setup-complete' -and $checklistSource -match 'no-android-crash-dialog'
